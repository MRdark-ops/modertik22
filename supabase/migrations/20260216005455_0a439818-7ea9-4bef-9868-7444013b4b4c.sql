...
-- Drop ALL existing policies and recreate as PERMISSIVE (explicit)
...
CREATE POLICY "Admins can view all commissions" ON public.referral_commissions FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users can view own totp" ON public.user_totp FOR SELECT TO authenticated USING (auth.uid() = user_id);
